...
def _extract_broker_pods(output: str) -> List[Tuple[str, str]]:
    pods: List[Tuple[str, str]] = []
    if not output:
        return pods
    for line in output.splitlines():
        if "broker" not in line.lower():
            continue
        parts = line.split()
        if len(parts) >= 2:
            namespace = parts[0]
...
def run_broker_logs_chain(context: Dict[str, Any]) -> Dict[str, Any]:
    """
    Broker 日志收集链：
    1) kubectl 查询全部 broker pod
    2) 逐个拉取 broker 容器日志
    """
    ...
    pods = _extract_broker_pods(list_result.get("output", ""))

...
def _pick_pid(jps_output: str) -> Optional[str]:
    for line in jps_output.splitlines():
        if "BrokerStartup" in line or "broker" in line.lower():
...
def run_broker_jvm_chain(context: Dict[str, Any]) -> Dict[str, Any]:
    """
    Broker JVM 诊断链：定位 Java 进程并采集 JVM 堆栈与进程信息。
    需要：namespace、pod、container（默认 broker）
    """
